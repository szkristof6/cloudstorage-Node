...
const Table = ({pageID, setPageID, files,loading, setSelected, folders, changeSort, settings}) => {
                ...
                <div className="list-body">
                    {
                        folders ? folders.map(dir => (
                            <Link key={dir._id} className="row" onClick={e => e.preventDefault()} onDoubleClick={() => {
                                ...
                                <div className="be-text" title={`Tipus: Mappa`}>Mappa</div>
                                <div className="be-text" title={`Létrehozva ${dir.user} által ${getDate(dir.createdAt)}`}>{getDate(dir.createdAt)}</div>
                    ...
                    }
                    {
                        files ? files.map(file => (
                            <Link key={file._id} className="row" onClick={e => e.preventDefault()} onDoubleClick={() => {
                                ...
                                <div className="be-text" title={`Tipus: ${file.meta.name}`}>{file.meta.name}</div>
                                <div className="be-text" title={`Létrehozva ${file.user} által ${getDate(file.createdAt)}`}>{getDate(file.createdAt)}</div>
                                <div className="be-text" title={`Méret: ${humanReadableByte(file.meta.size)}`}>{humanReadableByte(file.meta.size)}</div>
                            </Link>
                        )) : 'Töltés...'
                    }
                </div>
